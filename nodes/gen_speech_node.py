    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "voice": (
                    [
                        # todo: can be made dynamic but its per model possibly so has to be changed when model input is changed
                        # todo: see if ComfyUI NodeSpec v3 arrives before this gets more models and then see if v3 is easier to deal with this
                        "af_alloy",
                        "af_aoede",
                        "af_bella",
                        "af_heart",
